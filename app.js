...
function showWelcome(){hideAllViews();const w=document.getElementById('welcome');if(w)w.style.display='block';['controls','stats','toc'].forEach(function(id){const e=document.getElementById(id);if(e)e.style.display=id==='controls'?'flex':'block'});clearResumeBanner();var st=getSettings();var th=st.theme||'classic';var tEl=document.getElementById('wlcTitle');var dEl=document.getElementById('wlcDesc');var themeKey='theme'+(th.charAt(0).toUpperCase()+th.slice(1));if(tEl)tEl.textContent=t(th==='sudan'?'sudanWelcome':th==='festive'?'festiveGreeting':th==='rasta'?'rastaGreeting':'classicGreeting');if(dEl)dEl.innerHTML='<small style="opacity:.7;display:block;margin-bottom:4px">🎨 '+t('themeLabel')+': '+t(themeKey)+'</small>'+t('welcomeDesc');var sBtn=document.getElementById('sudanReadToggle');if(sBtn)sBtn.textContent='📖 '+t('readAbout');setTimeout(showDedication,500);}

function showDedication(){var d=document.getElementById('dedicationOverlay');if(d)d.remove();d=document.createElement('div');d.id='dedicationOverlay';d.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.65);z-index:10000;display:flex;align-items:center;justify-content:center;direction:rtl';d.innerHTML='<div style="background:var(--surface,#fff);border-radius:18px;padding:40px 32px;max-width:420px;width:90%;text-align:center;box-shadow:0 15px 50px rgba(0,0,0,.4);margin:20px;animation:fadeIn .5s"><div style="font-size:56px;margin-bottom:12px;line-height:1">🕯️</div><h2 style="color:var(--accent,#27ae60);margin:0 0 12px;font-size:20px">بسم الله الرحمن الرحيم</h2><p style="font-size:18px;line-height:2;color:var(--text,#333);margin:0">إلى روح والدي الغالي</p><p style="font-size:26px;font-weight:bold;color:var(--accent,#27ae60);margin:8px 0;line-height:1.4">ابراهيم محمد عبد الفضيل</p><p style="font-size:14px;color:var(--text-light,#888);margin:16px 0 0;line-height:1.8">اللهم اغفر له وارحمه<br>وأسكنه فسيح جناتك<br>واجعل هذا العمل في ميزان حسناته</p><button onclick="document.getElementById(\'dedicationOverlay\').remove()" style="margin-top:24px;padding:12px 40px;border:none;border-radius:10px;background:var(--accent,#27ae60);color:#fff;font-size:16px;cursor:pointer;font-family:inherit">اللهم آمين</button></div>';document.body.appendChild(d);}
